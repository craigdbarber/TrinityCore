        ...
        if (difficulty != 0)
        {
            ...
            if (player->GetLevel() <= dunLevel + SolocraftLevelDiff) //If a player is too high level for dungeon don't buff but if in a group will count towards the group offset balancing.
            {
                ...
                if (difficulty > 0)
                {
                    ...
                    //ss << "|cffFF0000[SoloCraft] |cffFF8000 {} entered {}  - Difficulty Offset: %0.2f. Spellpower Bonus: {}. Class Balance Weight: {}";
                    //ChatHandler(player->GetSession()).PSendSysMessage(ss.str().c_str(), player->GetName(), map->GetMapName(), difficulty, SpellPowerBonus, classBalance);
                }
                else
                {
